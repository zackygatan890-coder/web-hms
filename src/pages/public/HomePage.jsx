      ...
      <section className="py-24 bg-neutral-50 w-full relative overflow-hidden">
        <div className="container mx-auto px-6 md:px-12">
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
               ...
               <Link to="/publikasi" className="bg-white border-2 border-neutral-100 px-8 py-4 rounded-full text-xs font-black uppercase tracking-widest hover:border-emerald-500 hover:text-emerald-600 transition shadow-sm whitespace-nowrap">Lihat Papan Mading Seutuhnya</Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
               {(data.mading || []).slice(0, 3).map((item, id) => (
                 <div key={id} onClick={()=>setDetailModal({open:true, item, type:'mading'})} className="bg-white rounded-3xl overflow-hidden shadow-lg border border-neutral-100 cursor-pointer group">
                    ...
                    <div className="p-8">
                       <h3 className="text-xl font-black uppercase tracking-tighter italic mb-3 line-clamp-2">{item.title}</h3>
